%%
%This code simulates Fig. 5 in the paper "Utility Maximization ... Diversified Recommedation"
%%
clear
clc
...
meansize_content = 10;
rec_size_all = 2:10;
CHR_opt = zeros(1,length(rec_size_all));
CHR_Hete = zeros(1,length(rec_size_all));
CHR_Homo = zeros(1,length(rec_size_all));
...
for fa = 1:length(rec_size_all)   
    ...
    for it = 1:NumberIter
        ...
        CHR_opt(fa) = CHR_opt(fa)+sum(sum(a_req).*cache);
        ...
        rec_hete = zeros(K,N);
        for k = 1:K
            rec_homo(k,:) = top_rec(pref_sum,R(k),bar_a(k));
            rec_hete(k,:) = top_rec(a_pref(k,:),R(k),bar_a(k));
        end
        a_req_homo = (x'*ones(1,N)).*rec_homo.*a_pref./(sum(rec_homo.*a_pref,2)*ones(1,N))+(1-x'*ones(1,N)).*(1-rec_homo).*a_pref./(sum((1-rec_homo).*a_pref,2)*ones(1,N));
        a_req_hete = (x'*ones(1,N)).*rec_hete.*a_pref./(sum(rec_hete.*a_pref,2)*ones(1,N))+(1-x'*ones(1,N)).*(1-rec_hete).*a_pref./(sum((1-rec_hete).*a_pref,2)*ones(1,N));
        CHR_Homo(fa) = CHR_Homo(fa)+sum(sum(a_req_homo).*cache);
        CHR_Hete(fa) = CHR_Hete(fa)+sum(sum(a_req_hete).*cache);
       
        it
        fa
    end    
    CHR_opt(fa) = CHR_opt(fa)/NumberIter;
    CHR_Homo(fa) = CHR_Homo(fa)/NumberIter;
    CHR_Hete(fa) = CHR_Hete(fa)/NumberIter;
end
save(sprintf('CHR_Homo.mat'),'CHR_Homo');
save(sprintf('CHR_Hete.mat'),'CHR_Hete');
save(sprintf('CHR_opt.mat'),'CHR_opt');

figure;plot(rec_size_all,CHR_opt,'k-',rec_size_all,CHR_Homo,'b--',rec_size_all,CHR_Hete,'r--')